...
function changeStyle(obj, code) {
    ...
    for (i = 0; i < chords.length; ++i) {
        ...
        switch (code) {
        case 'I':
            if (chordStyle.fontStyle == "italic")
                chordStyle.fontStyle = "normal";
            else
                chordStyle.fontStyle = "italic";
            break;
        case 'B':
            if (chordStyle.fontWeight == "bold")
                chordStyle.fontWeight = "normal";
            else
                chordStyle.fontWeight = "bold";
            break;
        case '()':
...
function render_tab_chords() {
    $("#lyric").each(function() {
        var regExp = /[^[\]]+(?=])|[]+(?=\))/g;
        var chordExp = new RegExp("(([A-G][b#]?)((7sus4|7sus2|sus4|sus2|dim|aug|add9|madd9|m7M|maj7|m9|m7|m6|m|7M|9|7|6|4|11|m11|m7b5)?))","g");
...
function bodyLoad() {
    chord = "(([A-G][b#]?)((7sus4|7sus2|sus4|sus2|dim|aug|add9|madd9|m7M|maj7|m9|m7|m6|m|7M|9|7|6|4|11|m11|m7b5)?(\\s*[-/]?\\s*)))";
    openBracket = "[\\[\\(]";
    closeBracket = "[\\]\\)]";
    ...
    document.querySelector('#lyric').innerHTML=document.querySelector('#lyric').innerHTML.replace(/(Tone .+?)\</ig,'$1<br><br><')
}
$(function() {
    bodyLoad();
    render_tab_chords();
...
$(document).ready(function(e) {
    ...
    $('body').on('click','#fontsize', function(e) {
        var ourText = $("#lyric");
